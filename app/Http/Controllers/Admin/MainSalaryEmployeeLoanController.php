...
class MainSalaryEmployeeLoanController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.mainSalaryRecordLoan.index', ['financeMonthlyCalendars' => $financeMonthlyCalendars]);
    }
    ...
    public function ajaxCheck(Request $request)
    {
        if ($request->ajax()) {
            $company_id = Auth::user()->company_id;
            $checkIfExistsCounter = get_count_where(MainSalaryEmployeeLoan::class, ['company_id' => $company_id, 'finance_monthly_calendar_id' => $request->finance_monthly_calendar_id, 'employee_id' => $request->employee_id]);
    ...
    public function show($id)
    {
        ...
        if (empty($financeMonthlyCalendar)) {
            return redirect()->route('admin.main-salary-employee-loans.index')->with('error', 'عفوا غير قادر للوصول الى بيانات الشهر');
        }
        ...
        $employees_has_opened_monthly_record = Employee::select([
            ...
            ->orderBy('id', 'asc')
            ->get();
        $mainSalaryEmployeeLoans = MainSalaryEmployeeLoan::with([
            'employee',
        ...
        $mainSalaryEmployeeLoans2 = MainSalaryEmployeeLoan::where('company_id', $company_id)
            ->where('finance_monthly_calendar_id', $id)
            ->orderBy('id', 'desc')
            ->get();
        return view('admin.mainSalaryRecordLoan.show', [
            'financeMonthlyCalendar'          => $financeMonthlyCalendar,
            'mainSalaryEmployeeLoans'         => $mainSalaryEmployeeLoans,
            'mainSalaryEmployeeLoans2'        => $mainSalaryEmployeeLoans2,
            'employees'                       => $employees,
    ...
    public function ajaxSearch(Request $request)
    {
        if ($request->ajax()) {
            ...
            $mainSalaryEmployeeLoans = MainSalaryEmployeeLoan::with([
                ...
                ->orderBy('id', 'desc')
                ->paginate(PAGEINATION_COUNTER);
            return view('admin.mainSalaryRecordLoan.ajaxSearch', ['mainSalaryEmployeeLoans' => $mainSalaryEmployeeLoans]);
        }
    }
    ...
    public function edit(Request $request)
    {
        ...
        return response()->json(['status' => 'true', 'mainSalaryEmployeeLoans' => $mainSalaryEmployeeLoans]);
    }
    ...
    public function printSearch(Request $request)
    {
        ...
        $total_sum = $mainSalaryEmployeeLoans->sum('amount');

        return view('admin.mainSalaryRecordLoan.print_search', [
            'mainSalaryEmployeeLoans' => $mainSalaryEmployeeLoans,
