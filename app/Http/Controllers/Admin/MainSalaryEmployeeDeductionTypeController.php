...
class MainSalaryEmployeeDeductionTypeController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin.mainSalaryRecordDeductionType.index', ['financeMonthlyCalendars' => $financeMonthlyCalendars]);
    }
    ...
    public function ajaxCheck(Request $request)
    {
        if ($request->ajax()) {
            $company_id = Auth::user()->company_id;
            $checkIfExistsCounter = get_count_where(MainSalaryEmployeeDeductionType::class, ['company_id' => $company_id, 'finance_monthly_calendar_id' => $request->finance_monthly_calendar_id, 'employee_id' => $request->employee_id, 'deduction_type_id' => $request->deduction_type_id]);
    ...
    public function show($id)
    {
        ...
        if (empty($financeMonthlyCalendar)) {
            return redirect()->route('admin.main-salary-employee-deduction-types.index')->with('error', 'عفوا غير قادر للوصول الى بيانات الشهر');
        }
        ...
        $deductionTypes = DeductionType::where('company_id', $company_id)->orderBy('id', 'asc')->get();

        $mainSalaryEmployeeDeductionTypes = MainSalaryEmployeeDeductionType::with([
            'employee',
        ...
        $mainSalaryEmployeeDeductionTypes2 = MainSalaryEmployeeDeductionType::where('company_id', $company_id)
            ->where('finance_monthly_calendar_id', $id)
            ->orderBy('id', 'desc')
            ->get();

        return view('admin.mainSalaryRecordDeductionType.show', [
            'financeMonthlyCalendar'          => $financeMonthlyCalendar,
            'mainSalaryEmployeeDeductionTypes'    => $mainSalaryEmployeeDeductionTypes,
            'mainSalaryEmployeeDeductionTypes2'   => $mainSalaryEmployeeDeductionTypes2,
            'employees'                       => $employees,
    ...
    public function ajaxSearch(Request $request)
    {
        if ($request->ajax()) {
            ...
            return view('admin.mainSalaryRecordDeductionType.ajaxSearch', ['mainSalaryEmployeeDeductionTypes' => $mainSalaryEmployeeDeductionTypes]);
        }
    }
    ...
    public function destroy(Request $request)
    {
        if ($request->ajax()) {
            ...
            $mainSalaryEmployeeDeductionType = getColsWhereRow(MainSalaryEmployeeDeductionType::class, ['id'], ['company_id' => $company_id, 'id' => $request->id, 'is_archived' => 0]);
            if (empty($mainSalaryEmployeeDeductionType)) {
                return response()->json(['status' => 'false', 'message' => 'عفوا غير قادر للوصول الى بيانات الخصم']);
            }
    ...
    public function edit(Request $request)
    {
        ...
        $mainSalaryEmployeeDeductionType = MainSalaryEmployeeDeductionType::with([
        ...
        return response()->json(['status' => 'true', 'mainSalaryEmployeeDeductionType' => $mainSalaryEmployeeDeductionType]);
    }
    ...
    public function update(Request $request)
    {
        if ($request->ajax()) {
            ...
            $mainSalaryEmployeeDeductionType = getColsWhereRow(MainSalaryEmployeeDeductionType::class, ['*'], ['company_id' => $company_id, 'id' => $request->id]);
            if (empty($mainSalaryEmployeeDeductionType)) {
                return response()->json(['status' => 'false', 'message' => 'عفوا غير قادر للوصول الى بيانات الخصم']);
            }
    ...
    public function printSearch(Request $request)
    {
        ...
        $total_amount_sum = $mainSalaryEmployeeDeductionTypes->sum('amount');

        return view('admin.mainSalaryRecordDeductionType.print_search', [
            'mainSalaryEmployeeDeductionTypes' => $mainSalaryEmployeeDeductionTypes,
