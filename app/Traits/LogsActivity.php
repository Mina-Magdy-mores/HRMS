...
trait LogsActivity
{
    ...
    protected static function logAction($model, $actionName)
    {
        try {
            ...
            // If the model exists and has an ID, make sure all attributes are loaded (in case select list was limited)
            if ($model->exists && isset($model->id)) {
                $attributes = $model->getAttributes();
                $fullRecord = \Illuminate\Support\Facades\DB::table($model->getTable())
                    ->where('id', $model->id)
                    ->first();
                if ($fullRecord) {
    ...
    public function getModuleName()
    {
        $map = [
            ...
            \App\Models\MainSalaryEmployeeSettlement::class => 'تسويات رواتب الموظفين المؤرشفة',
            \App\Models\SalaryGrantType::class => 'أنواع منح الرواتب',
            \App\Models\DirectBonus::class => 'المكافئات المباشرة',
            \App\Models\DirectGrant::class => 'المنح المباشرة',
