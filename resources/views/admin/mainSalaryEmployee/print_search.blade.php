...
<head>
    ...
    <title>طباعة رواتب الموظفين لشهر {{ optional(optional($financeMonthlyCalendar)->month)->name }}</title>
    ...
    <style>
        ...
        .summary-card.danger .sc-value {
            color: #c0392b;
        }

        .summary-card.success .sc-value {
            color: #27ae60;
        }
...
<body>
    ...
    <div class="report-title-box">
        <h3>كشف رواتب الموظفين المفصل</h3>
    ...
    @if ($mainSalaryEmployees->count() > 0)
        ...
        <div class="summary-section">
            <div class="summary-card info">
                ...
                <div class="sc-value">{{ $mainSalaryEmployees->count() }}</div>
            </div>
            <div class="summary-card">
                <div class="sc-label">إجمالي الرواتب الأساسية</div>
                <div class="sc-value">{{ number_format($total_salary_sum, 2) }} ج.م</div>
            </div>
            <div class="summary-card success">
                <div class="sc-label">إجمالي الاستحقاقات</div>
                <div class="sc-value">{{ number_format($total_benefits_sum, 2) }} ج.م</div>
            </div>
            <div class="summary-card danger">
                <div class="sc-label">إجمالي الاستقطاعات</div>
                <div class="sc-value">{{ number_format($total_deductions_sum, 2) }} ج.م</div>
            </div>
            <div class="summary-card info">
                <div class="sc-label">إجمالي صافي الرواتب</div>
                <div class="sc-value font-weight-bold" style="color: #2c3e50;">
                    {{ number_format($total_net_salary_sum, 2) }} ج.م</div>
            </div>
        </div>
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($record->is_disbursed == 1)
                                <span class="badge-type badge-paid">تم الصرف</span>
                            @else
                                <span class="badge-type badge-pending">لم يصرف</span>
                            @endif
                        </td>
                    </tr>
                @endforeach
