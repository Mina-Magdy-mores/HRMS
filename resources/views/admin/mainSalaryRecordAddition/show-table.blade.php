<div class="container-fluid">
            ...
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h5 class="mb-0 text-dark font-weight-bold">
                        ...
                        أضافه أيام شهر: <span class="text-primary">{{ $financeMonthlyCalendar->month->name }}</span>
    ...
    <div class="row mb-4">
        ...
        <div class="col-lg-3 col-md-6 col-12">
            <div class="info-box shadow-sm">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">إجمالي عدد الإضافات</span>
                    <span class="info-box-number">{{ $mainSalaryEmployeeAdditions2->count() }}</span>
                </div>
            </div>
        </div>
        <div class="col-lg-3 col-md-6 col-12">
            <div class="info-box shadow-sm">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">الإضافات غير المؤرشفة</span>
                    <span class="info-box-number">
                        {{ $mainSalaryEmployeeAdditions2->where('is_archived', 0)->count() }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-3 col-md-6 col-12">
            <div class="info-box shadow-sm">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">الإضافات المؤرشفة</span>
                    <span class="info-box-number">
                        {{ $mainSalaryEmployeeAdditions2->where('is_archived', 1)->count() }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-3 col-md-6 col-12">
            <div class="info-box shadow-sm">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">إجمالي المبالغ المضافة</span>
                    <span class="info-box-number text-success font-weight-bold">
                        {{ number_format($mainSalaryEmployeeAdditions2->sum('total'), 2) }}
                        <small>ج.م</small>
                    </span>
                </div>
            </div>
    ...
    <div class="card card-primary card-outline shadow">
        ...
        <div class="card-body">
            ...
            @endif
            <form action="{{ route('admin.main-salary-employee-additions.print-search') }}" method="POST"
                target="_blank">
                @csrf
...
<!-- Add Modal -->
<div class="modal fade " id="addMainSalaryRecordAdditionModal" tabindex="0" role="dialog" aria-hidden="true">
        ...
        <div class="modal-content shadow">
            ...
            <div class="modal-body" id="months_modal_body">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <button type="submit" class="btn btn-success shadow px-4" id="submit_add_addition">
...
<!-- Edit Modal -->
<div class="modal fade " id="editMainSalaryRecordAdditionModal" tabindex="0" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
        <div class="modal-content shadow">
            ...
            <div class="modal-body" id="edit_months_modal_body">
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <input type="hidden" id="edit_addition_id" name="id">
                        <input type="hidden" id="edit_main_salary_employee_id" name="main_salary_employee_id">
                        <button type="submit" class="btn btn-success shadow px-4" id="submit_edit_addition">
                            <i class="fas fa-save"></i>
                            حفظ البيانات
                        </button>
                        <button type="button" class="btn btn-danger shadow px-4" data-dismiss="modal">الغاء</button>
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#submit_add_addition', function(e) {
                ...
                $.ajax({
                    url: "{{ route('admin.main-salary-employee-additions.ajax-check') }}",
                    dataType: "json",
                    cache: false,
                    method: "POST",
                    data: {
                    ...
                    success: function(response) {
                        if (response.status == 'true') {
                            var res = confirm(' يوجد عدد  ' + response.count +
                                '  إضافات لنفس الموظف ونفس الشهر هل تريد الاضافة');
                            if (res == true) {
                                var flag = true;
                            } else {
                                var flag = false;
                        ...
                        if (flag) {
                            $.ajax({
                                url: "{{ route('admin.main-salary-employee-additions.store') }}",
                                dataType: "json",
                                cache: false,
                                method: "POST",
                                data: {
                                ...
                                success: function(response) {
                                    if (response.status == 'true') {
                                        ...
                                        $('#addMainSalaryRecordAdditionModal')
                                            .modal('hide');
            ...
            function ajax_search() {
                ...
                $.ajax({
                    url: '{{ route('admin.main-salary-employee-additions.ajax-search') }}',
                    type: 'POST',
                    dataType: 'html',
                    cache: false,
                    data: {
                    ...
                    success: function(mainSalaryEmployeeAdditions) {
                        $('#ajax_responce_search').html(mainSalaryEmployeeAdditions);
                    },
                    error: function(xhr) {}
                });
            }
            ...
            $(document).on('click', '#delete-addition-btn', function() {
                var id = $(this).data('id');
                var main_salary_employee_id = $(this).data('main-salary-employee-id');
                var res = confirm('هل انت متاكد من حذف هذه الاضافة');
                if (res == true) {
                    $.ajax({
                        url: "{{ route('admin.main-salary-employee-additions.destroy') }}",
                        type: 'POST',
                        dataType: 'json',
                        cache: false,
                        data: {
            ...
            $(document).on('click', '.edit-addition', function() {
                ...
                var main_salary_employee_id = $(this).data('main-salary-employee-id');
                $.ajax({
                    url: "{{ route('admin.main-salary-employee-additions.edit') }}",
                    type: 'POST',
                    dataType: 'json',
                    data: {
                        _token: '{{ csrf_token() }}',
                    ...
                    success: function(response) {
                        if (response.status == 'true') {
                            var addition = response.mainSalaryEmployeeAdditions;
                            var employee = addition.employee;
                            var modal = $('#editMainSalaryRecordAdditionModal');

                            modal.find("#edit_addition_id").val(addition.id);
                            modal.find("#edit_main_salary_employee_id").val(addition.main_salary_employee_id);
                            ...
                            modal.find("#edit_days_amount").val(addition.days_amount);
                            modal.find("#edit_total").val(addition.total);
                            modal.find("#edit_notes").val(addition.notes);
            ...
            $(document).on('click', '#submit_edit_addition', function(e) {
                var id = $('#edit_addition_id').val();
                ...
                var payment_per_day = $('#edit_payment_per_day').val();
                $.ajax({
                    url: "{{ route('admin.main-salary-employee-additions.update') }}",
                    type: 'POST',
                    dataType: 'json',
                    cache: false,
                    data: {
                    ...
                    success: function(response) {
                        if (response.status == 'true') {
                            alert(response.message);
                            $('#editMainSalaryRecordAdditionModal').modal('hide');
                            ajax_search();
                        } else {
