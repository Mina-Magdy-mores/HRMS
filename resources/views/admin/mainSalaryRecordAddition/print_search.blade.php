...
<head>
    ...
    <title>طباعة إضافات الأيام الشهرية للموظفين</title>
...
<body>
    ...
    <div class="report-title-box">
        <h3>كشف إضافات الأيام الشهرية للموظفين</h3>
    ...
    <div class="meta-row">
        ...
        <span>📋 <strong>إجمالي السجلات:</strong> {{ $mainSalaryEmployeeAdditions->count() }}</span>
    </div>

    @if($mainSalaryEmployeeAdditions->count() > 0)

        {{-- ===== SUMMARY CARDS ===== --}}
        <div class="summary-section">
            <div class="summary-card info">
                <div class="sc-label">عدد الإضافات</div>
                <div class="sc-value">{{ $mainSalaryEmployeeAdditions->count() }}</div>
            </div>
            <div class="summary-card">
                <div class="sc-label">إجمالي الأيام المضافة</div>
                <div class="sc-value">{{ number_format($total_days_sum, 2) }} يوم</div>
            </div>
            <div class="summary-card success">
                <div class="sc-label">إجمالي المبالغ المضافة</div>
                <div class="sc-value">{{ number_format($total_sum, 2) }} ج.م</div>
            </div>
            <div class="summary-card">
                <div class="sc-label">إضافات مؤرشفة</div>
                <div class="sc-value">{{ $mainSalaryEmployeeAdditions->where('is_archived', 1)->count() }}</div>
            </div>
            <div class="summary-card">
                <div class="sc-label">إضافات غير مؤرشفة</div>
                <div class="sc-value">{{ $mainSalaryEmployeeAdditions->where('is_archived', 0)->count() }}</div>
            </div>
        </div>
        ...
        <table class="print-table">
            ...
            <tbody>
                @foreach ($mainSalaryEmployeeAdditions as $index => $addition)
                <tr>
                    {{-- رقم --}}
                    <td>{{ $index + 1 }}</td>

                    {{-- كود الموظف --}}
                    <td>{{ optional($addition->employee)->employee_code ?? '---' }}</td>

                    {{-- اسم الموظف --}}
                    <td style="text-align:right; font-weight:bold;">
                        {{ optional($addition->employee)->name ?? '---' }}
                    </td>

                    {{-- عدد الأيام --}}
                    <td>{{ number_format($addition->days_amount, 2) }}</td>
                    ...
                    {{-- أضيف بواسطة --}}
                    <td>{{ optional($addition->addedBy)->name ?? '---' }}</td>

                    {{-- تاريخ الإضافة --}}
                    <td style="white-space:nowrap;">
                        {{ $addition->created_at ? $addition->created_at->format('Y-m-d h:i:s A') : '---' }}
                    </td>

                    {{-- آخر تعديل --}}
                    <td style="white-space:nowrap;">
                        @if($addition->updated_at && $addition->updated_at != $addition->created_at)
                            {{ $addition->updated_at->format('Y-m-d h:i:s A') }}
                            @if(optional($addition->updatedBy)->name)
                                <br><small style="color:#777;">{{ $addition->updatedBy->name }}</small>
                            @endif
                        @else
                            ---
                        @endif
