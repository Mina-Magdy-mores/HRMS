...
@section('title', 'تعديل المنحة المباشرة')
...
@section('contentHeader')
    <i class="fas fa-gift text-primary"></i> تعديل المنحة المباشرة
@endsection
    ...
    <div class="card card-primary card-outline shadow">
        <div class="card-header bg-white">
            <h3 class="card-title font-weight-bold text-dark mt-1">
                <i class="fas fa-edit mr-1 text-primary"></i> تعديل بيانات المنحة المالية المباشرة خارج المرتب
            </h3>
        ...
        <form action="{{ route('admin.direct-grants.update', $directGrant->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                @if ($errors->any())
                ...
                <div class="row">
                    <div class="col-md-6 form-group">
                        ...
                        <select name="employee_id" id="employee_id" class="form-control select2 @error('employee_id') is-invalid @enderror">
                            ...
                            @foreach ($employees as $emp)
                                <option value="{{ $emp->id }}" {{ old('employee_id', $directGrant->employee_id) == $emp->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 form-group">
                        <label class="font-weight-bold">الشهر المالي للمنحة <span class="text-danger">*</span></label>
                ...
                <div class="row mt-2">
                    <div class="col-md-4 form-group">
                        ...
                        <select name="salary_grant_type_id" id="salary_grant_type_id" class="form-control select2 @error('salary_grant_type_id') is-invalid @enderror">
                            ...
                            @foreach ($grantTypes as $type)
                                <option value="{{ $type->id }}" {{ old('salary_grant_type_id', $directGrant->salary_grant_type_id) == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        <div class="input-group">
                            <input type="number" name="amount" step="0.01" min="0.01" value="{{ old('amount', $directGrant->amount) }}"
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        <input type="date" name="payment_date" value="{{ old('payment_date', $directGrant->payment_date) }}"
                ...
                <div class="row mt-2">
                    <div class="col-md-8 form-group">
                        ...
                        <textarea name="notes" rows="2" class="form-control" placeholder="اكتب أي ملاحظات إضافية هنا...">{{ old('notes', $directGrant->notes) }}</textarea>
                    </div>

                    <div class="col-md-4 form-group">
                        <label class="font-weight-bold">الحالة <span class="text-danger">*</span></label>
                        <select name="status" class="form-control @error('status') is-invalid @enderror">
                            <option value="1" {{ old('status', $directGrant->status) == 1 ? 'selected' : '' }}>مفعل</option>
                            <option value="0" {{ old('status', $directBonus->status ?? 0) == 0 ? 'selected' : '' }}>معطل</option>
                        </select>
                        @error('status')
                            <span class="invalid-feedback">{{ $message }}</span>
                        @enderror
