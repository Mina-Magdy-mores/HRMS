...
@section('title', 'تسجيل مكافأة مباشرة')
...
@section('contentHeader')
    <i class="fas fa-gift text-primary"></i> تسجيل مكافأة مباشرة
@endsection
    ...
    <div class="card card-primary card-outline shadow">
        <div class="card-header bg-white">
            <h3 class="card-title font-weight-bold text-dark mt-1">
                <i class="fas fa-plus-circle mr-1 text-primary"></i> تسجيل مكافأة مباشرة جديدة خارج المرتب
            </h3>
        ...
        <form action="{{ route('admin.direct-bonuses.store') }}" method="POST">
            @csrf
            <div class="card-body">
                @if ($errors->any())
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        <select name="bonus_id" id="bonus_id" class="form-control select2 @error('bonus_id') is-invalid @enderror">
                            ...
                            @foreach ($bonusTypes as $type)
                                <option value="{{ $type->id }}" {{ old('bonus_id') == $type->id ? 'selected' : '' }}>
                                    {{ $type->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="card-footer bg-light text-left">
                <button type="submit" class="btn btn-success shadow px-4 font-weight-bold">
                    <i class="fas fa-save mr-1"></i> حفظ المكافأة
                </button>
