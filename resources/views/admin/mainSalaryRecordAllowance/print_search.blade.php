...
<head>
    ...
    <title>طباعة البدلات الشهرية للموظفين</title>
...
<body>
    ...
    <div class="report-title-box">
        <h3>كشف البدلات الشهرية للموظفين</h3>
    ...
    <div class="meta-row">
        ...
        <span>📋 <strong>إجمالي السجلات:</strong> {{ $mainSalaryEmployeeAllowances->count() }}</span>
    </div>

    @if($mainSalaryEmployeeAllowances->count() > 0)

        {{-- ===== SUMMARY CARDS ===== --}}
        <div class="summary-section">
            <div class="summary-card info">
                <div class="sc-label">عدد البدلات</div>
                <div class="sc-value">{{ $mainSalaryEmployeeAllowances->count() }}</div>
            </div>
            <div class="summary-card success">
                <div class="sc-label">إجمالي مبالغ البدلات</div>
                <div class="sc-value">{{ number_format($total_amount_sum, 2) }} ج.م</div>
            </div>
            <div class="summary-card">
                <div class="sc-label">بدلات مؤرشفة</div>
                <div class="sc-value">{{ $mainSalaryEmployeeAllowances->where('is_archived', 1)->count() }}</div>
            </div>
            <div class="summary-card">
                <div class="sc-label">بدلات غير مؤرشفة</div>
                <div class="sc-value">{{ $mainSalaryEmployeeAllowances->where('is_archived', 0)->count() }}</div>
            </div>
        </div>
        ...
        <table class="print-table">
            ...
            <tbody>
                @foreach ($mainSalaryEmployeeAllowances as $index => $allowance)
                <tr>
                    {{-- رقم --}}
                    <td>{{ $index + 1 }}</td>

                    {{-- كود الموظف --}}
                    <td>{{ optional($allowance->employee)->employee_code ?? '---' }}</td>

                    {{-- اسم الموظف --}}
                    <td style="text-align:right; font-weight:bold;">
                        {{ optional($allowance->employee)->name ?? '---' }}
                    </td>

                    {{-- نوع البدل --}}
                    <td>{{ optional($allowance->allowanceType)->name ?? '---' }}</td>
                    ...
                    {{-- أضيف بواسطة --}}
                    <td>{{ optional($allowance->addedBy)->name ?? '---' }}</td>

                    {{-- تاريخ الإضافة --}}
                    <td style="white-space:nowrap;">
                        {{ $allowance->created_at ? $allowance->created_at->format('Y-m-d h:i:s A') : '---' }}
                    </td>

                    {{-- آخر تعديل --}}
                    <td style="white-space:nowrap;">
                        @if($allowance->updated_at && $allowance->updated_at != $allowance->created_at)
                            {{ $allowance->updated_at->format('Y-m-d h:i:s A') }}
                            @if(optional($allowance->updatedBy)->name)
                                <br><small style="color:#777;">{{ $allowance->updatedBy->name }}</small>
                            @endif
                        @else
                            ---
                        @endif
            ...
            <tfoot>
                <tr>
                    ...
                    <td style="color:#27ae60;">{{ number_format($total_amount_sum, 2) }} ج.م</td>
                    <td colspan="6"></td>
                </tr>
            </tfoot>
        </table>
