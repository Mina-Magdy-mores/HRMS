...
<head>
    ...
    <title>طباعة المكافآت الشهرية المجمعة للموظفين</title>
...
<body>
    ...
    <div class="report-title-box">
        <h3>كشف المكافآت المالية الشهرية للموظفين</h3>
    ...
    <div class="meta-row">
        ...
        <span>📋 <strong>إجمالي السجلات:</strong> {{ $mainSalaryEmployeeBonuses->count() }}</span>
    </div>

    @if($mainSalaryEmployeeBonuses->count() > 0)

        {{-- ===== SUMMARY CARDS ===== --}}
        <div class="summary-section">
            <div class="summary-card info">
                <div class="sc-label">عدد المكافآت</div>
                <div class="sc-value">{{ $mainSalaryEmployeeBonuses->count() }}</div>
            </div>
            <div class="summary-card success">
                <div class="sc-label">إجمالي مبالغ المكافآت</div>
                <div class="sc-value">{{ number_format($total_amount_sum, 2) }} ج.م</div>
            </div>
            <div class="summary-card">
                <div class="sc-label">مكافآت مؤرشفة</div>
                <div class="sc-value">{{ $mainSalaryEmployeeBonuses->where('is_archived', 1)->count() }}</div>
            </div>
            <div class="summary-card">
                <div class="sc-label">مكافآت غير مؤرشفة</div>
                <div class="sc-value">{{ $mainSalaryEmployeeBonuses->where('is_archived', 0)->count() }}</div>
            </div>
        </div>
        ...
        <table class="print-table">
            ...
            <tbody>
                @foreach ($mainSalaryEmployeeBonuses as $index => $bonus)
                <tr>
                    {{-- رقم --}}
                    <td>{{ $index + 1 }}</td>

                    {{-- كود الموظف --}}
                    <td>{{ optional($bonus->employee)->employee_code ?? '---' }}</td>

                    {{-- اسم الموظف --}}
                    <td style="text-align:right; font-weight:bold;">
                        {{ optional($bonus->employee)->name ?? '---' }}
                    </td>

                    {{-- نوع المكافأة --}}
                    <td>{{ optional($bonus->bonus)->name ?? '---' }}</td>
                    ...
                    <td style="white-space:nowrap;">
                        {{ $bonus->created_at ? $bonus->created_at->format('Y-m-d h:i:s A') : '---' }}
                    </td>

                    {{-- آخر تعديل --}}
                    <td style="white-space:nowrap;">
                        @if($bonus->updated_at && $bonus->updated_at != $bonus->created_at)
                            {{ $bonus->updated_at->format('Y-m-d h:i:s A') }}
                            @if(optional($bonus->updatedBy)->name)
                                <br><small style="color:#777;">{{ $bonus->updatedBy->name }}</small>
                            @endif
                        @else
                            ---
                        @endif
            ...
            <tfoot>
                <tr>
                    ...
                    <td style="color:#27ae60; font-weight:bold;">{{ number_format($total_amount_sum, 2) }} ج.م</td>
                    <td colspan="6"></td>
                </tr>
            </tfoot>
        </table>
