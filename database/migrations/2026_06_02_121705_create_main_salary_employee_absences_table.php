...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('main_salary_employee_absences', function (Blueprint $table) {
            ...
            $table->foreign('finance_monthly_calendar_id', 'fk_absence_calendar') // اسم قصير
                ->references('id')
                ->on('finance_monthly_calendars')
                ->onUpdate('cascade');
            $table->decimal('days_amount', 10, 2)->comment('days amount of absence');
            $table->decimal('total', 10, 2)->comment('total salary for absence days');
