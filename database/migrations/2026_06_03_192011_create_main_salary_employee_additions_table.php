...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('main_salary_employee_additions', function (Blueprint $table) {
            ...
            $table->foreign('finance_monthly_calendar_id', 'fk_addition_calendar')
                ->references('id')
                ->on('finance_monthly_calendars')
                ->onUpdate('cascade');
            $table->decimal('days_amount', 10, 2)->comment('days amount of addition');
            $table->decimal('total', 10, 2)->comment('total salary for addition days');
